...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	const singleTransaction = await db
		...
		.where(and(eq(transactions.branchId, locals?.user?.branch), eq(transactions.id, id)))
		.groupBy(
			transactions.id,
			transactions.createdAt,
			transactions.amount,
	...
	const soldProducts = await db
		.select({
			...
			soldBy: sql<string>`
      TRIM(CONCAT(${employee.firstName}, ' ', COALESCE(${employee.lastName}, '')))
    `,
    ...
    `
		...
		.leftJoin(employee, eq(transactionProducts.staffId, employee.id)) // <-- REQUIRED
