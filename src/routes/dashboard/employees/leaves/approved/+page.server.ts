...
export const load: PageServerLoad = async () => {
	...
	const leaves = await db
		...
		.where(eq(leave.status, 'approved'));
	...
	approve: async ({ request, locals, params }) => {
		...
		try {
			await db.transaction(async (tx) => {
				// 1. Get the leave details before updating (so we can calculate days)
				...
				// 3. Refund days if status is pending or rejected
				if (status === 'pending' || status === 'rejected') {
					// Map to track total days to refund per staff member
					// (In case one staff member has multiple rejected leaves in this batch)
			...
			return message(form, {
				type: 'success',
				text: `Leave Status changed to ${status === 'pending' ? 'Pending' : 'Rejected'} Successfully!`
			});
		} catch (err) {
