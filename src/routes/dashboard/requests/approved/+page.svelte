			...
			<div>
				...
				>
					...
					{:else}
						...
						Download In Separate Files PDF
					{/if}
				</Button>
				...
				>
					...
					{:else}
						...
						Download In Separate Files PNG
					{/if}
				</Button>
			</div>
	...
	<div class="flex flex-col items-center gap-16 pb-20">
		{#each data?.requests as site, index}
			...
			<section
				...
				data-sitename="{site?.siteName?.replace(/\s+/g, '-')}-{site.month}-{site.year}"
			>
				<!-- Header -->
				...
				<div class="mb-10 grid grid-cols-1 gap-12 border-y border-zinc-100 py-8">
					...
					<div class="">
						...
						<p class="text-sm font-semibold">
							...
							<span class="font-bold">{site.year} </span> ወርሃዊ/የአንድ ጊዜ ክፍያ ወርሃዊ/የአንድ ጊዜ ክፍያ እንደሚከተለው እንጠ
							ይቃለን፡፡
						</p>
					</div>
				</div>
