...
import { eq, and, isNull, sql, desc, getTableColumns } from 'drizzle-orm';
...
export const load: PageServerLoad = async ({ params, locals }) => {
	const requests = await db
		...
		.where(eq(paymentRequest.status, 'approved'));
	...
	const sitesList = await db
		...
		.innerJoin(siteContracts, eq(site.id, siteContracts.siteId));
	...
	request: async ({ request, locals }) => {
		...
		try {
			...
			const existingRequest = await db
				.select()
				.from(paymentRequest)
				.where(and(eq(paymentRequest.month, monthName), eq(paymentRequest.year, Number(year))))
				.limit(1)
				.then((result) => result[0]);
			if (existingRequest) {
				setError(form, 'month', `A request for ${monthName} ${year} already exists.`);
				return message(form, {
					type: 'error',
					text: `A request for ${monthName} ${year} already exists.`
				});
			}

			const result = await db.transaction(async (tx) => {
				const insertable = contracts.map((id, index) => ({
					siteId: id,
					invoiceNumber: invoiceNumbers[index],
					requestAmount: requestAmounts[index],
					penality: penalityAmounts[index],
					vat: vat,
					withhold: withhold,
					requestDate,
